...
impl LiquidationMonitor {
    ...
    pub async fn start_monitoring(&self) -> Result<()> {
        info!("🔍 Starting liquidation monitoring for pool: {}", self.pool_address);
        info!("📊 Monitoring started at: {}", Local::now().format("%Y-%m-%d %H:%M:%S"));
    ...
    async fn log_liquidation(&self, event: &LiquidationEvent) {
        ...
        info!("📊 Collateral Liquidated: {} wei", event.liquidated_collateral_amount);
        ...
        if event.liquidated_collateral_amount > event.debt_to_cover {
            ...
            info!("💎 Liquidation Bonus: ~{}% (approximate)", bonus_percent / U256::from(100));
        }
    ...
    pub async fn print_stats_summary(&self) {
        ...
        if stats.total_liquidations > 0 {
            ...
            info!("🏦 Total Collateral Liquidated: {} wei", stats.total_collateral_liquidated);
            info!("👥 Unique Liquidators: {}", stats.unique_liquidators.len());
            info!("🎯 Unique Users Liquidated: {}", stats.unique_users_liquidated.len());
            ...
            if hours > 0 {
                ...
                info!("\n⚡ Average Frequency: {:.2} liquidations/hour", avg_per_hour);
            }
            
            if let Some(last_time) = stats.last_liquidation_time {
                info!("🕐 Last Liquidation: {}", last_time.format("%Y-%m-%d %H:%M:%S UTC"));
            }
        } else {
