...
pub fn init_asset_configs() -> HashMap<Address, AssetConfig> {
    ...
    tracing::info!("Initialized {} asset configuration(s) for oracle monitoring", configs.len());
    tracing::info!("🎯 Active oracle feeds:");
    for config in configs.values() {
        tracing::info!("   {} ({}): {}", config.symbol, config.address, config.chainlink_feed);
    }
