...
use crate::models::{LiquidationOpportunity, LiquidationParams, HardhatArtifact};
    ...
    P: Provider,
{
    ...
    async fn call_liquidate_function(&self, params: &LiquidationParams) -> Result<String> {
        ...
        let tx_req = call.into_transaction_request();

        // Add gas settings
        let mut tx_req = tx_req
            .gas_limit(800_000) // High gas limit for complex liquidation
            .gas_price(self.provider.get_gas_price().await? * 2); // 2x gas price for priority

        // Sign and send transaction
        let signed_tx = self.signer.sign_transaction(&mut tx_req).await?;
    ...
    async fn wait_for_confirmation(&self, tx_hash: &str) -> Result<()> {
        ...
        while attempts < MAX_ATTEMPTS {
            ...
            match self.provider.get_transaction_receipt(hash).await {
                Ok(Some(receipt)) => {
                    if receipt.status() {
                        info!("✅ Transaction confirmed successfully in block: {:?}", receipt.block_number);
                        return Ok(());
                    } else {
                ...
                Ok(None) => {
                    ...
                    attempts += 1;
                    if attempts % 15 == 0 {
                        info!("⏳ Still waiting for confirmation... ({}/{})", attempts, MAX_ATTEMPTS);
                    }
                }
                Err(e) => {
    ...
    pub async fn verify_contract_setup(&self) -> Result<()> {
        ...
        if let Some(pool_address) = result.first() {
            if let Some(addr) = pool_address.as_address() {
                ...
                let expected_pool: Address = "0x07eA79F68B2B3df564D0A34F8e19D9B1e339814b".parse()?;
                if addr == expected_pool {
                    info!("✅ Pool address verification successful");
                } else {
                    warn!("⚠️ Pool address mismatch - expected: {:?}, got: {:?}", expected_pool, addr);
                }
            }
        }
